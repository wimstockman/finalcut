...
// Create input fields
	...
	this->ES.push_back("tester");
	this->ES.push_back("tester5");
	this->ES.push_back("tester7");
 	for(int i=0;i< this->ES.size();i++){
		FLineEdit* test_field = new FLineEdit{ES[i],this}; 
		test_field->setGeometry(FPoint(9,(i*3+20)),FSize(20,1));
		test_field->setLabelText(this->ES[i]);
		test_field->redraw();
		LE.push_back(test_field);
		
	}
	this->redraw();
	this->show();
 	for(int i=0;i< this->LE.size();i++){
	std::cerr<<LE[i]->getText()<<" : "<<i;
	}
//	this->close();
	}
